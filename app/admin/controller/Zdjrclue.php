    ...
    public function index(){
        if (request()->isPost()) {
            ...
            if(!empty($post_data['time_range'])){
                ...
                $clue_ids = $this->logM->getField('clue_id', ['create_time' => ['between', [strtotime($range[0]), strtotime($range[1])]]]);
                $where['clue.id'] = ['in', count($clue_ids) ? $clue_ids : [0]];
            }
        ...
        $builder->addTopButton('addnew')
            ->setSearch([
                ...
                ['type' => 'datetime_range', 'name' => 'time_range', 'title' => '搜索时间']
            ])
