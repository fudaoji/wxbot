 ...
 * Description: 小浣熊（基于千寻）驱动
 * Link: https://apifox.com/apidoc/project-2808564/api-85593035
...
class Xhx extends Base
{
    ...
    {
        empty($params['content']) && $params['content'] = $params['msg'];
        $to_wxid = is_array($params['to_wxid']) ? $params['to_wxid'] : explode(',', $params['to_wxid']);
        foreach($to_wxid as $id){
            $params['to_wxid'] = $id;
