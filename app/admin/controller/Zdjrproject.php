...
class Zdjrproject extends Botbase
{
    ...
    public function initialize()
    {
        ...
        $this->projectBotM = new ProjectBot();
    }
    ...
    public function index(){
        ...
        $builder->addTopButton('addnew')
            ...
            ->addRightButton('edit')
            ->addRightButton('edit', ['title' => '绑定机器人', 'href' => url('bind', ['project_id' => '__data_id__'])])
    ...
    public function bind(){
        if(request()->isPost()){
            $post_data = input('post.');
            $bots = explode(',', $post_data['bots']);
            if(!count($bots)){
                $this->error('请选择机器人');
            }
            foreach ($bots as $bot_id){
                if(! $this->projectBotM->total(['admin_id' => $this->adminInfo['id'],'bot_id' => $bot_id, 'project_id' => $post_data['project_id']])){
                    $this->projectBotM->addOne(['admin_id' => $this->adminInfo['id'],'bot_id' => $bot_id, 'project_id' => $post_data['project_id']]);
                }
            }
        ...
        $builder = new FormBuilder();
        $builder->setPostUrl(url('bind'))
            ->addFormItem('project_id', 'hidden', 'id', 'id')
            ->addFormItem('title', 'static', '项目', $data['title'])
            ->addFormItem('bots', 'chosen_multi', '选择机器人', '选择机器人', $bots, 'required')
            ->setFormData($data);
