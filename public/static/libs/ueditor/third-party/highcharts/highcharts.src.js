		...
		// setter
			...
			for (key in hash) {
				...
				if (result !== false) {
					...
					// is unable to cast them. Banner again with final IE9.
					} else if (key === 'width') {
						value = pInt(value);
...
Highcharts.VMLElement = VMLElement = {
	...
	setSpanRotation: function (rotation, sintheta, costheta) {
		...
		// Banner case: http://highcharts.com/tests/?file=text-rotation
		css(this.element, {
...
Tooltip.prototype = {
	...
	getPosition: function (boxWidth, boxHeight, point) {
		...
		// Banner to see if the tooltip is too far to the right,
...
var AreaSeries = extendClass(Series, {
	...
	getSegmentPath: function (segment) {
		...
		if (options.stacking && !this.closedStacks) {
			...
			// User stack back. Todo: implement areaspline. A general solution could be to
