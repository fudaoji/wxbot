...
class Emoji
{
    ...
    /**
     * to convert Docomo data into the unified format
     * @param $text
     * @return mixed
     * Author: fudaoji<fdj@kuryun.cn>
     */
    function emojiDocomoToUnified($text)
    {
        return $this->emojiConvert($text, 'docomo_to_unified');
    }

    /**
     * to convert kddi data into the unified format
     * @param $text
     * @return mixed
     * Author: fudaoji<fdj@kuryun.cn>
     */
    function emojiKddiToUnified($text)
    {
        return $this->emojiConvert($text, 'kddi_to_unified');
    }

    /**
     * to convert softbank data into the unified format
     * @param $text
     * @return mixed
     * Author: fudaoji<fdj@kuryun.cn>
     */
    function emojiSoftbankToUnified($text)
    {
        return $this->emojiConvert($text, 'softbank_to_unified');
    }

    /**
     * to convert google data into the unified format
     * @param $text
     * @return mixed
     * Author: fudaoji<fdj@kuryun.cn>
     */
    function emojiGoogleToUnified($text)
    {
        return $this->emojiConvert($text, 'google_to_unified');
    }


    /**
     * to convert unified data into the docomo  format
     * @param $text
     * @return mixed
     * Author: fudaoji<fdj@kuryun.cn>
     */
    function emojiUnifiedToDocomo($text)
    {
        return $this->emojiConvert($text, 'unified_to_docomo');
    }

    /**
     * to convert unified data into the kddi  format
     * @param $text
     * @return mixed
     * Author: fudaoji<fdj@kuryun.cn>
     */
    function emojiUnifiedToKddi($text)
    {
        return $this->emojiConvert($text, 'unified_to_kddi');
    }

    /**
     * to convert unified data into the softbank  format
     * @param $text
     * @return mixed
     * Author: fudaoji<fdj@kuryun.cn>
     */
    function emojiUnifiedToSoftbank($text)
    {
        return $this->emojiConvert($text, 'unified_to_softbank');
    }

    /**
     * to convert unified data into the google  format
     * @param $text
     * @return mixed
     * Author: fudaoji<fdj@kuryun.cn>
     */
    function emojiUnifiedToGoogle($text)
    {
        return $this->emojiConvert($text, 'unified_to_google');
    }


    /**
     * to convert unified data into the html  format
     * @param $text
     * @return mixed
     * Author: fudaoji<fdj@kuryun.cn>
     */
    ...
    /**
     * to convert html data into the unified  format
     * @param $text
     * @return mixed
     * Author: fudaoji<fdj@kuryun.cn>
     */
    function emojiHtmlToUnified($text)
    {
        return preg_replace_callback("!<span class=\"emoji-outer emoji-sizer\"><span class=\"emoji-inner emoji([0-9a-f]+)\"></span></span>!", function ($m) {
            if (isset($this->emojiMaps['html_to_unified'][$m[1]])) {
                return $this->emojiMaps['html_to_unified'][$m[1]];
            }
            return $m[0];
        }, $text);
    }
    ...
    function emojiConvert($text, $map)
    {
        return str_replace(array_keys($this->emojiMaps[$map]), $this->emojiMaps[$map], $text);
    }

    function emojiGetName($unified_cp)
    {
        return $this->emojiMaps['names'][$unified_cp] ? $this->emojiMaps['names'][$unified_cp] : '?';
    }

    function emojiContainsEmoji($text)
    {
        $count = 0;
        str_replace($this->emojiMaps['prefixes'], '00', $text, $count);
        return $count > 0;
    }

    /**
     * convert hex into unified
     * @param $text
     * @return string
     * Author: fudaoji<fdj@kuryun.cn>
     */
    function hexToUnified($text){
        $arr = is_array($text) ? $text : [$text];
        $hex = [];
        foreach ($arr as $unified) {
            $hex[] = sprintf('U+%04X', $unified);
        }
        return implode(' ', $hex);
    }
     ...
     * Author: fudaoji<fdj@kuryun.cn>
     */
    function unifiedToHex($text){
        $utf = str_replace("U+", "0x", $text);
        sscanf($utf, "%x", $hex);
        return  $hex;
    }
