...
class Admingroup extends Base
{
    ...
    public function auth() {
        if(request()->isPost()) {
            ...
            $result = model('AdminGroup')->updateOne($update_data);
            if($result) {
        ...
        $data = model('AdminGroup')->getOne($group_id);
        if(! $data) {
            $this->error('id非法');
        }
    ...
    public function getRulesTree() {
        if(request()->isPost()){
            $post_data = input('post.');
            $rules = model('AdminRule')->getAll([
                'where' => ['status' => 1],
                'order' => ['sort' => 'asc'],
                'field' => 'id, pid, title, href'
            ]);
            $group = model('AdminGroup')->getOne($post_data['group_id']);
            ...
            foreach ($rules as &$item) {
                ...
                $total = model('AdminRule')->total(['pid' => $item['id'], 'status' => 1]);
