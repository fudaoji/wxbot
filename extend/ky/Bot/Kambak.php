...
class Kambak extends Base
{
    ...
    {
        $this->host = empty($options['host']) ? '127.0.0.1' : $options['host'];
        $this->port = empty($options['port']) ? '8090' : $options['port'];
    ...
    public function index(){
        ...
        Logger::error(json_encode($data, JSON_UNESCAPED_UNICODE));
