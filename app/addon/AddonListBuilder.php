...
class AddonListBuilder extends AddonController
{
    ...
    private $_template = 'builder/list';                    // 模版
    ...
    public function addTopButton($type, $attribute = null) {
        $set_status_url = $this->urlPrefix . 'setstatus';
        switch ($type) {
            case 'addnew':  // 添加新增按钮
                // 预定义按钮属性以简化使用
                ...
                $my_attribute['href']  = addon_url($this->urlPrefix . 'add');
            ...
            case 'delete': // 添加删除按钮(我没有反操作，删除了就没有了，就真的找不回来了)
                ...
                $my_attribute['href']  = addon_url($set_status_url, ['status' => 'delete']);
    ...
    public function addRightButton($type, $attribute = null) {
        ...
        switch ($type) {
            case 'edit':  // 编辑按钮
                ...
                $my_attribute['href']  = addon_url($this->urlPrefix . 'edit', ['id' => '__data_id__'], '');
